...
<x-layouts.app>
    ...
    <section class="relative overflow-hidden bg-zinc-900">
        ...
        <div class="relative z-10 mx-auto max-w-6xl px-4 py-20 text-left sm:py-28">
            <div class="max-w-3xl">
                @if ($heroImageTitle)
                    <p class="text-xs font-bold uppercase tracking-[0.25em] {{ $hasHeroImage ? 'text-emerald-400' : 'text-emerald-600' }}">
                        {{ FilamentFlexibleContentBlocks::replaceParameters($heroImageTitle) }}
                    </p>
                @endif

                @if ($title)
                    <h1 class="mt-4 text-4xl font-bold tracking-tight text-white sm:text-5xl lg:text-6xl drop-shadow-sm {{ $hasHeroImage ? 'text-white' : 'text-zinc-900' }}">
                        {{ FilamentFlexibleContentBlocks::replaceParameters($title) }}
                    </h1>
                @endif

                @if ($introText)
                    <div class="mt-6 text-lg leading-relaxed sm:text-xl {{ $hasHeroImage ? 'text-zinc-200 [&_a]:text-white [&_a]:underline' : 'text-zinc-700 [&_a]:text-zinc-900 [&_a]:underline' }}">
        ...
        @if ($heroImageCopyright)
            <small class="absolute bottom-3 right-4 z-10 rounded-md bg-black/50 px-2.5 py-1 text-xs text-white/80 backdrop-blur-sm">
